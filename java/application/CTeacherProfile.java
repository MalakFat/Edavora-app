...
public class CTeacherProfile implements Initializable {

    @FXML
    private TextField emailF, fnameF, lnameF,salary;
    @FXML
    ...
    private void loadUserData() {
        ...
        String sql = """
            SELECT u.email, u.firstname, u.lastname, u.password, u.birthdate, u.gender, u.profile_image,
                   t.salary
            FROM public."User" u
            LEFT JOIN public.teacher t ON u.email = t.email
            WHERE u.email = ?
            """;
        ...
        try (Connection conn = connectDB();
            ...
            if (rs.next()) {
                ...
                salary.setText(rs.getString("salary") != null ? rs.getString("salary") : "");
            } else {
    ...
    private void handleSave() {
        ...
        String sqlUser = "UPDATE public.\"User\" SET firstname=?, lastname=?, birthdate=?, gender=?, profile_image=? WHERE email=?";
        String sqlTeacher = "INSERT INTO public.teacher (email, salary) VALUES (?, ?) ON CONFLICT (email) DO UPDATE SET salary = EXCLUDED.salary";
        ...
        try (Connection conn = connectDB()) {
            ...
            try (PreparedStatement pstmt2 = conn.prepareStatement(sqlTeacher)) {
                pstmt2.setString(1, email);

                if (!salary.getText().isEmpty()) {
                    pstmt2.setBigDecimal(2, new java.math.BigDecimal(salary.getText()));
                } else {
                    pstmt2.setNull(2, Types.NUMERIC);
                }
    ...
    private void handleChangePassword() {
        try {
            ...
            Parent root = FXMLLoader.load(getClass().getResource("UTeacherChangePassword.fxml"));
